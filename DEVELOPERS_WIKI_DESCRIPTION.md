...
1. Clients hit the FastAPI adapter on port 8001 using Tavily-style `/search` and `/extract` calls.
...
- **Production / shared environments (recommended):**
  ...
  - Verify with `curl http://localhost:8001/health` and a sample `/search` request.
...
- **Topology:** One Azure Container App hosts three containers (adapter, Redis, SearXNG) that share the same VNet namespace; only port 8001 from the adapter faces the public ingress.
...
- Common troubleshooting: ensure Docker ports 8001 (adapter) and 8999 (SearXNG) are free, and confirm Redis is reachable when rate limiting misbehaves.
